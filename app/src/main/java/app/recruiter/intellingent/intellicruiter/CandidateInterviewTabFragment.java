...
public class CandidateInterviewTabFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.tab_fragment_interview_candidate, container, false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mRecyclerViewApplication = (RecyclerView) view.findViewById(R.id.recycler_view_candidate_accepted_applications);
    ...
    public void onStart() {
        ...
        databaseApplications.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                showAcceptedApplication(users, clients, requirements, candidates, applications);
            }
    ...
    public void showAcceptedApplication(List<User> user, List<Client> client, List<Requirement> requirement, List<Candidate> candidate, List<Application> application) {

        List<Application> application_accepted = new ArrayList<>();
        firebaseAuth = FirebaseAuth.getInstance();
        String loggedInUser = firebaseAuth.getCurrentUser().getEmail();

        int intialsize = application.size();
        for(int i =0; i<intialsize;i++)
        {
            if(application.get(i).getStatus().compareTo("ACCEPTED")==0 && application.get(i).getCandidate().getCandidateEmail().compareTo(loggedInUser) ==0){
                application_accepted.add(application.get(i));
            }
        }

        //creating recyclerview adapter
        mAdapterApplication = new AcceptedApplicationAdapter(user, requirement,client,application_accepted,candidate);
